...
I_hor = 7 #Strom zum kompensieren des Erdmagnetfeldes (A)
...
print(spez_2)
#Plots
plt.plot(B(I_1),V_1,'kx',label='Messwerte bei U=250V')
plt.plot(B(I_2),V_2,'bx',label='Messwerte bei U=380V')
plt.plot(B(I_1),gerade(B(I_1),noms(a_1),noms(b_1)),label='Ausgleichsgerade 250V')
plt.plot(B(I_2),gerade(B(I_2),noms(a_2),noms(b_2)),label='Ausgleichsgerade 380V')
plt.grid()
plt.legend()
plt.show()
