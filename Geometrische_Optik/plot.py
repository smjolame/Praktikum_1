...
for linse in [150,300]:
    # Messwerte einlesen
    g,b = np.genfromtxt('data/gb{}.csv'.format(linse), delimiter=',', unpack=True)
    ...
    # Tabelle
    data = list(zip(g,b,f))
    np.savetxt('data/gb{}_tabelle.csv'.format(linse), data, header='g[cm],b[cm],f[cm]', fmt='%i,%2.1f,%2.2f')
    ...
    # Plot der Daten
    plt.plot(g, g*0, 'go', markersize=3, label='Gegenstandsweiten $g$')
    plt.plot(b*0, b, 'bo', markersize=3, label='Bildweiten $b$')
    ...
    # Plot der Schnittpunkte
    plt.plot(s_x,s_y, 'r.', markersize=5, markeredgewidth=0.1, label="Schnittpunkte")
